...
options_file = "https://s3-us-west-2.amazonaws.com/allennlp/models/elmo/2x4096_512_2048cnn_2xhighway/elmo_2x4096_512_2048cnn_2xhighway_options.json"  # noqa: E501
weight_file = "https://s3-us-west-2.amazonaws.com/allennlp/models/elmo/2x4096_512_2048cnn_2xhighway/elmo_2x4096_512_2048cnn_2xhighway_weights.hdf5"  # noqa: E501
...
actv_emb = (
    elmo(batch_to_ids([[v] for v in actv]))["elmo_representations"][0]
    .detach()
    .numpy()  # noqa: E501
)
stav_emb = (
    elmo(batch_to_ids([[v] for v in stav]))["elmo_representations"][0]
    .detach()
    .numpy()  # noqa: E501
)
...
class Sentence:
    def __init__(self, sent):
        ...
        self.emb = (
            elmo(batch_to_ids(self.raw_sent))["elmo_representations"][0]
            .detach()
            .numpy()
        )
    ...
    def encode(self):
        ...
        self.encode_quality()
        code = re.sub(
            "[\.,-]", "", ("".join([x.code for x in self.sent]))
        )  # noqa: W605
        return code
        ...
        while i < self.len:
            for j in range(self.len, i, -1):
                ...
                if phrase in ga:
                    ...
                    self.sent[i:j] = [
                        t.replace_text(tr)
                        for t, tr in zip(self.sent[i:j], ga[phrase].split())
                    ]
                    self.sent[i].code = "n"
                    self.sent[i + 1 : j] = [  # noqa: E203
                        t.replace_code(tr)
                        for t, tr in zip(
                            self.sent[i + 1 : j],  # noqa: E203
    ...
    def encode_verb(self):
        for i, token in enumerate(self.sent):
            if token.code == "." and token.pos in VERB_TAG:
                word_emb = self.emb[i]
                stav_similar = sklearn.metrics.pairwise.cosine_similarity(
                    stav_emb.squeeze(), word_emb
                ).max()
                actv_similar = sklearn.metrics.pairwise.cosine_similarity(
                    actv_emb.squeeze(), word_emb
                ).max()
    ...
    def encode_preposition(self):
        for i, token in enumerate(self.sent):
            if (
                token.code == "."
                and token.pos in PREP_TAG
                and token.dep in ["case", "amod"]
                and token.depid != 0
                and self.sent[token.depid - 1].code == "n"
            ):
                token.code = "r"
    ...
    def encode_quality(self):
        for i, token in enumerate(self.sent):
            ...
            if (
                token.code == "."
                and token.pos in ADJ_TAG
                and i + 1 < self.len
                and self.sent[i + 1].code == "t"
            ):
                token.code = "q"
