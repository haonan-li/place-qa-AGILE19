...
def process_data(data,ga,abbr,pt_set,pt_dict,actv,stav,sp_prep):
    for item in tqdm(data):
        ...
        analyze = process_sentence(query,ga,abbr,pt_set,pt_dict,actv,stav,sp_prep)
        item['queryAnalyze'] = analyze
        ## process answer
        if 'answers' in item:
            item['answersAnalyze'] = list()
            for answer in item['answers']:
                analyze = process_sentence(answer.lower(),ga,abbr,pt_set,pt_dict,actv,stav,sp_prep)
        ...
        if (code[i] == '.'):
            ...
            # verb (simple dict lookup)
                ...
                if stem in actv:
                    ...
                    stative.append(stem)
            # preposition
            elif (tag[i][1] in PREP_TAG) and (token in sp_prep) and (i+1<query_len) and (code[i+1] == 'n'):
                code[i] = 'r'
